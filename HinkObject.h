#ifndef HINKOBJECT_H
#define HINKOBJECT_H

#include <QObject>
#include <QMetaType>
#include <QMutex>
#include <QList>
#include <QVariantMap>
#include <QThread>
#include <QReadWriteLock>

class Hink;
class HinkObject : public QObject
{
    Q_OBJECT
public:
    friend class Hink;
    explicit HinkObject(QObject *parent = 0);
    struct Packet{
        char *data;
        int len;
        qint64 pts;
        qint64 dts;
        enum  PacketType{
            IFrame, PFrame, BFrame, AFrame, SPFrame, SPFrame_info
        }type;
        HinkObject *sender;
        Packet(){data=NULL;len=0;pts=0;sender=NULL;dts=-1;}
    };

    struct StreamInfo{
        enum DataType{
            UnKnow, VPSS, Raw, FLTP, H264, H265, MJPEG, PCMA, PCMU, G722, AAC, MP2, MPEGTS, RTSP, MPEG4, MP3, AC3, OPUS
        }type;
        QVariantMap info;
        HinkObject *sender;
        StreamInfo(){type=UnKnow;sender=NULL;}
    };



    struct Buffer{
        QReadWriteLock lock;
        QList<Packet> buffer;
        int len;
        Buffer(): lock(QReadWriteLock::Recursive){

        }
    };

//    void destroy();
    HinkObject* linkA(HinkObject* dst);
    HinkObject* linkV(HinkObject* dst);
//    HinkObject* linkE(HinkObject* dst);
//    HinkObject* unLinkA(HinkObject* dst);
//    HinkObject* unLinkV(HinkObject* dst);
//    HinkObject* unLinkE(HinkObject* dst);
//    QVariant invoke(QString func, QVariant value=QVariant());
    enum State{
        created,started,stoped,destroyed
    }state;
    Q_ENUM(State)

    StreamInfo getStreamInfoA();
    StreamInfo getStreamInfoV();
protected:
    QVariantMap data,dataTemp,dataUser;
    StreamInfo infoSrcA,infoSrcV,infoSelfA,infoSelfV,infoATemp,infoVTemp;
    Buffer bufferA,bufferV;
    QStringList directFunc;
    QString threadName;
    QThread::Priority threadPriority;
protected:
    char* bufferData(char *data,int size,int offset=0);
//    void sendPacketA(Packet pkt);
    void sendPacketV(Packet pkt);
//    void sendInfoA(StreamInfo info);
    void sendInfoV(StreamInfo info);
//    void sendEvent(QString type, QVariant msg);
//    bool bufferPacketA(Packet pkt);
//    bool bufferPacketV(Packet pkt);
    void memInit(int size);
private:
    char *memPool;
    char *memPtr;
    int memSize;
//    void invokeInfoA(StreamInfo info);
//    void invokeInfoV(StreamInfo info);
signals:
    void newEvent(QString type, QVariant msg);
    void newPacketA(Packet pkt);
    void newPacketV(Packet pkt);
    void newInfoA(StreamInfo info);
    void newInfoV(StreamInfo info);
public slots:
//    bool setData(QString json);
    bool setData(QVariantMap data);
//    const QVariantMap getData();
//    const QVariantMap getUserData();
    bool start();
      bool start(QVariantMap data);
//    bool start(QString json);
//    bool stop(bool block=false);
    QString name();
    QString getState();

//    virtual void onNewPacketA(Packet);
    virtual void onNewPacketV(Packet);
//    virtual void onUnLinkA(StreamInfo);
//    virtual void onUnLinkV(StreamInfo);
//    void onUnLinkAPriv(StreamInfo info);
//    void onUnLinkVPriv(StreamInfo info);
//    virtual void onNewEvent(QString type,QVariant info);
private slots:
//    void onNewInfoAPriv(StreamInfo info);
//    void onNewInfoVPriv(StreamInfo info);
//    void onSetDataPriv(QVariantMap data);
//    QVariantMap doGetData();
//    virtual void onNewInfoA(StreamInfo);
    virtual void onNewInfoV(StreamInfo);
    virtual void onSetData(QVariantMap);
    virtual void onStart();
//    virtual void onStop();
};

#endif // HINKOBJECT_H
